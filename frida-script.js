...
var config = {
  package: "com.margento.slovenskezeleznice",
  printSummary: true,
      ...
      overload.implementation = function () {
        ...
        if (config.printArgs) {
          ...
          for (var arg of arguments) {
            if (arg !== null && arg !== undefined) {
              log(levels.warn, `\t\t- Args[${i}]: ${arg.toString()}`);
            }
        ...
        if (config.printReturn) {
          if (ret !== null && ret !== undefined) {
            log(levels.warn, `\t\t- Return: ${ret.toString()}`);
          }
        }
        return ret;
      };
  ...
  Java.perform(function () {
    Java.choose("u11", {
      onMatch: (instance) => {
        ...
        instance.getRequest.implementation = function (request) {
          const body = this.getRequest(request);
          console.log(body);
          return body;
        };
        return instance;
...
Java.perform(function () {
  ...
  const JBase64 = Java.use("android.util.Base64");
  const JStandardCharsets = Java.use("java.nio.charset.StandardCharsets");
  const cryptoUtil = Java.use(
    "com.margento.slovenskezeleznice.util.CryptoUtil"
  );
  // Decryption
  cryptoUtil.a.implementation = function (encrypted, context) {
    const jsonByteArray = this.a(encrypted, context);
    const json = JString.$new(jsonByteArray);
    console.log("Response decrypted: ", json);
    console.log("Response encrypted: ", JBase64.encodeToString(encrypted, 2));

    return jsonByteArray;
  };
  // Encryption
  cryptoUtil.b.implementation = function (json, context) {
    const encrypted = this.b(json, context);
    console.log("Request decrypted: ", json);
    console.log("Request encrypted: ", JString.$new(encrypted));
    return encrypted;
  };

  cryptoUtil.r.implementation = function (authnticateDeviceResult) {
    this.r(authnticateDeviceResult);
    console.log("TOKEN: ", authnticateDeviceResult.getToken());
  };
});
